...
/*pub fn apply_true_stereo_reverb(buffer: &mut [f32], sample_rate: u32) {
    ...
    for i in 0..n_frames {
        ...
        for line in 0..num_lines {
            let d_l = comb_l[line][idx_l[line]];
            let d_r = comb_r[line][idx_r[line]];
            out_l += d_l * comb_norm;
            out_r += d_r * comb_norm;

            let in_l = left[i] + d_l * decays[line] + d_r * decays[line] * 0.2;
            let in_r = right[i] + d_r * decays[line] + d_l * decays[line] * 0.2;

            comb_l[line][idx_l[line]] = in_l;
            comb_r[line][idx_r[line]] = in_r;
    ...
    // Allpass diffuser (4 tahap, kiri/kanan independen)
...
/*
/// Mengubah voice mono menjadi stereo interleaved dengan Haas delay
/// dan low‑pass filter ringan untuk menghindari comb filtering.
pub fn make_stereo_voice(mono: &[f32], width: f32, sample_rate: u32) -> Vec<f32> {
    let len = mono.len();
    let mut stereo = vec![0.0f32; len * 2];
    let width = width.clamp(0.0, 1.0);
    
    // Haas delay maksimum ~1.5 ms pada 44.1 kHz ≈ 66 sampel
    let haas_max = (1.5 * sample_rate as f32 / 1000.0) as usize;
    let haas_offset = (haas_max as f32 * width) as usize;
    
    // Low‑pass sederhana (one‑pole) untuk menghaluskan artifact
    let rc = 1.0 / (2.0 * PI * 12000.0); // cutoff 12 kHz
    let dt = 1.0 / sample_rate as f32;
    let alpha = dt / (rc + dt);
    
    // State filter kiri & kanan
    let mut lp_l = 0.0f32;
    let mut lp_r = 0.0f32;
    ...
    for i in 0..len {
        ...
        // Kiri: sample saat ini
        let left = center;
        // Kanan: sample yang di‑delay (Haas)
        let right = if haas_offset > 0 {
            let idx = i.saturating_sub(haas_offset);
            mono[idx]
        } else {
        ...
        // Terapkan low‑pass pada kiri dan kanan
        lp_l = lp_l + alpha * (left - lp_l);
        lp_r = lp_r + alpha * (right - lp_r);
        
        // Gain center agar energi tetap (equal power)
        let gain = 0.5f32.sqrt();
        stereo[i * 2] = lp_l * gain;
        stereo[i * 2 + 1] = lp_r * gain;
    }
